...
import { ExternalLink, Edit, Share2, Check, Triangle } from "lucide-react";
...
const groupEventsByDate = (events: NormalizedEvent[], rawEvents: any[]) => {
...
function RouteComponent() {
	...
	const getConfidenceBadge = (score: number | undefined) => {
		...
		return (
			<Badge className={`ml-2 ${color}`}>{(score * 100).toFixed(0)}%</Badge>
		);
	};
	...
	const discoveredAt = organization.discoveredAt
		? new Date(organization.discoveredAt)
		: null;
	const lastScrapedAt = organization.lastScrapedAt
		? new Date(organization.lastScrapedAt)
		: null;
	const lastPostAt = organization.lastPostAt
		? new Date(organization.lastPostAt)
		: null;
				...
				{isEditMode && (
					<div className="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
						<p className="text-sm text-yellow-800 dark:text-yellow-200">
							Edit functionality coming soon. This will allow you to modify
							organization fields, update metadata, and save changes.
						</p>
					</div>
				)}
			</div>
